// Cloudflare Worker - TrendOracle API
// Real-time data collection from YouTube, Google Trends, News
...
async function searchTrends(keyword, env, headers) {
  ...
  const result = await env.DB.prepare(`
    INSERT INTO trends (keyword, category, growth_rate, confidence, youtube_views, chart_data)
    VALUES (?, ?, ?, ?, ?, ?)
  ...
  `).bind(
    ...
    score.confidence,
    score.youtubeViews,
    JSON.stringify(score.chartData)
  ).first();
...
async function generatePrediction(keyword, env, headers) {
  ...
  const reasoning = `YouTube에서 "${keyword}" 관련 콘텐츠가 ${youtubeData.videoCount || 0}개 발견되었으며, 총 조회수는 ${(score.youtubeViews / 10000).toFixed(1)}만회입니다. 과거 유사 키워드의 성장 패턴을 분석한 결과, 향후 ${daysToPeak}일 내에 피크에 도달할 것으로 예측됩니다.`;
...
addEventListener('scheduled', event => {
  event.waitUntil(
    // Auto-collect data for popular keywords
    Promise.resolve()
  );
});
